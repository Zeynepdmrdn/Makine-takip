...
export function ProductionHistoryDialog({
  ...
  useEffect(() => {
    ...
    // Keeps the graph and table synchronized with demo production
  ...
  const statistics = useMemo(() => {
    ...
    const totalExpected = records.reduce((total, record) => total + record.expectedQuantity, 0);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby={`production-history-title-${workOrder.id}`}
    >
      <div className="mx-auto my-6 w-full max-w-6xl rounded-3xl bg-slate-100 p-6 shadow-2xl sm:p-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2
              id={`production-history-title-${workOrder.id}`}
        ...
        <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-2 flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500">{progress.toFixed(1)}%</span>

            <span
              className={`rounded-full px-3 py-1 text-xs font-semibold ${
                targetReached ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"
              }`}
            >
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="rounded-2xl bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-bold text-slate-900">{statistics.totalProduced}</p>
          </div>
        ...
        {!isLoading && records.length === 0 && (
          <p className="mt-5 rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-slate-500">
            No production has been recorded yet. Production will begin when the work order is
            active, the machine is RUNNING and Demo Simulation is started.
          </p>
        )}
          ...
          <>
            <section className="mt-5 rounded-2xl bg-white p-5 shadow-sm">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Expected output, actual output and deviation per demo interval.
                </p>
              </div>
